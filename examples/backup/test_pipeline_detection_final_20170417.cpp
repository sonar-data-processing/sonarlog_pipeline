...
bool isBadFrame(const cv::Mat& frame, const cv::Mat& mask) {
    ...
    cv::Mat top_left      = frame(cv::Rect(rect.x + 0 * rect.width / 3, rect.y, rect.width / 3, rect.height / 2));
    cv::Mat top_middle    = frame(cv::Rect(rect.x + 1 * rect.width / 3, rect.y, rect.width / 3, rect.height / 2));
    cv::Mat top_right     = frame(cv::Rect(rect.x + 2 * rect.width / 3, rect.y, rect.width / 3, rect.height / 2));
    cv::Mat bottom_left   = frame(cv::Rect(rect.x + 0 * rect.width / 3, rect.y + rect.height / 2, rect.width / 3, rect.height / 2));
    cv::Mat bottom_middle = frame(cv::Rect(rect.x + 1 * rect.width / 3, rect.y + rect.height / 2, rect.width / 3, rect.height / 2));
    cv::Mat bottom_right  = frame(cv::Rect(rect.x + 2 * rect.width / 3, rect.y + rect.height / 2, rect.width / 3, rect.height / 2));

    double avg_top_left      = cv::sum(top_left)[0] / top_left.total();
    double avg_top_middle    = cv::sum(top_middle)[0] / top_middle.total();
    double avg_top_right     = cv::sum(top_right)[0] / top_right.total();
    double avg_bottom_left   = cv::sum(bottom_left)[0] / bottom_left.total();
    double avg_bottom_middle = cv::sum(bottom_middle)[0] / bottom_middle.total();
    double avg_bottom_right  = cv::sum(bottom_right)[0] / bottom_right.total();

    return (avg_top_left < 0.16
            || avg_top_middle < 0.16
            || avg_top_right  < 0.16
            || avg_bottom_left < 0.16
            || avg_bottom_middle < 0.16
            || avg_bottom_right < 0.16);
}
...
int main(int argc, char const *argv[]) {
    ...
    for (size_t i = 0; i < num_logfiles; i++) {
        ...
        while (stream.current_sample_index() < stream.total_samples()) {
            ...
            if(average_frames.empty())
                average_frames = cv::Mat::zeros(cart_denoised.size(), cart_denoised.type());

            if (!pattern) {
                if (counter++ < 100)
                cv::accumulate(cart_denoised, average_frames);
                else {
                    average_frames = average_frames / counter;
                    pattern = true;
                }
            } else {
                cv::imshow("pattern", average_frames);
                cv::Mat cart_res =  cart_denoised - average_frames * 0.5;
                cv::imshow("res", cart_res);
                cv::Mat cart_debug2;
                cart_res.copyTo(cart_debug2, cart_roi);
                cv::imshow("debug2", cart_debug2);
            }

            /* evaluate and discard bad frames */
            // if(isBadFrame(cart_denoised, cart_roi)) continue;
