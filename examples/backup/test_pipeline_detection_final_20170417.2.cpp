...
bool double_check(const cv::Mat& src, const cv::Mat& mask, const std::pair<cv::Point2f, cv::Point2f>& best_model, int threshold = 15, int neighborhood = 20){
    bool found = false;
    double best_score = 1;
    ...
    double min_idx = -1, max_idx = - 1, old_diff = 0;
    for (size_t i = averages.size() / 2; i < averages.size(); i++) {
        double current_diff = std::abs( averages[neighborhood - 1] - averages[i]) / std::abs(neighborhood - 1 - i);
        std::cout << " current diff " << current_diff << " center " << neighborhood - 1 << " i " << i << std::endl;
    ...
    old_diff = 0;
    for (size_t i = averages.size() / 2 - 2 ; i > 0; i--) {
    ...
    return ((max_idx - min_idx) <= threshold);
}
...
int main(int argc, char const *argv[]) {
    ...
    for (size_t i = 0; i < num_logfiles; i++) {
        ...
        //
            ...
            bool found;
            if(scan_vertical){
                found = pipeline_detection(cart_filtered, cart_roi, best_model, 0.30, 45, 5, 5);
                if (found) {
