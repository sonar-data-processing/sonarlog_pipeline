...
bool double_check(const cv::Mat& src, const cv::Mat& mask, const std::pair<cv::Point2f, cv::Point2f>& best_model, int neighborhood = 20){
    bool found = false;
    double best_score = 1;
    ...
    cv::meanStdDev(cv::Mat(averages), mean, stddev);
    cv::Mat avgs = cv::Mat(averages) < (mean[0]);
    std::cout << "Mean and stdded: " << avgs.t() << std::endl;


    int center_idx;
    cv::minMaxIdx(cv::Mat(averages), NULL, NULL, &center_idx, NULL);

    int max_idx = averages.size(), min_idx = 0;
    double old_diff = 0;
    for (int i = center_idx + 1; i < averages.size(); i++) {
        // double current_diff = std::abs( averages[center_idx] - averages[i]) / std::abs(center_idx - i);
        double current_diff = std::abs(averages[i] - averages[center_idx]);
        std::cout << " current diff " << current_diff << " center " << center_idx << " i " << i << std::endl;
    ...
    std::cout << "Min/Max/Diff: " << min_idx << "/" << max_idx << "/" << (max_idx - min_idx)<< std::endl;

    return ((max_idx - min_idx) <= neighborhood);
}
...
int main(int argc, char const *argv[]) {
    ...
    for (size_t i = 0; i < num_logfiles; i++) {
        ...
        while (stream.current_sample_index() < stream.total_samples()) {
            ...
            cv::Mat cart_enhanced = insonificationCorrection(cart_corrected, cart_roi);

            cv::Rect roi = getMaskLimits(cart_roi);
            cv::Mat process = cart_enhanced(roi);
        ...
        //
            ...
            process.convertTo(process, CV_8U, 255);
            cv::blur(process, process, cv::Size(5,5));
            preprocessing::adaptive_clahe(process, process, 3);
            cv::imshow("process2", process);
            process.convertTo(process, CV_32F, 1.0 / 255);

            cv::Mat cart_filtered = cart_enhanced.clone();
            process.copyTo(cart_filtered(roi));
            ...
            if(scan_vertical){
                ...
                found = pipeline_detection(cart_filtered.t(), cart_roi.t(), best_model, 0.30, 45, 5, 5);
                if(found) {
